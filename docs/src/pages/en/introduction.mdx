...
Right now HuggerEngine is just in alpha. There are a lots of bugs and missings features. The plan for the 0.2 version is to support webgl. In the current version Huggerengine only support the canvas api. then webgl is supported it will be a lot faster and must higher fps.
